...
class DCTModel(BaseModel):
    ...
    def init_input(self):
        self.dp_max_anno = self.opt.dp_max_anno
        nb = self.batchSize
    ...
    def init_smpl(self):
        ...
        self.mean_param_file = osp.join(
            self.opt.pretrained_models_dir, 'neutral_smpl_mean_params.h5')
        self.load_mean_params()
        # set differential SMPL (implemented with pytorch) and smpl_renderer
        self.smpl_face_path = osp.join(
            self.opt.pretrained_models_dir, self.opt.smpl_face_file)
        self.smpl_model_path = osp.join(
            self.opt.pretrained_models_dir, self.opt.smpl_model_file)
