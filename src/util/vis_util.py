...
def recover_IUV(IUV):
    # normalize format
    if isinstance(IUV, torch.Tensor):
        if IUV.device.type == 'cuda':
            IUV = IUV.cpu().detach().numpy()
        else:
            IUV = IUV.detach().numpy()
    else:
        assert isinstance(IUV, np.ndarray)
...
def draw_dp_anno(image, dp_kps, dp_kps_weight, color=(0,0,255), img_size=224):
    image = recover_img(image)
    kps, kps_weight = recover_keypoint(dp_kps, dp_kps_weight, img_size)
    # recover color 
    if color == 'red':
        color = (0, 0, 255)
    elif color == 'green':
...
def render_image(vert, cam, image, inputSize, smpl_face_path):
    image = recover_img(image)
    renderer = SMPLRenderer(img_size=inputSize, face_path=smpl_face_path)
    rend_img = renderer(vert, cam, image)
    return rend_img
